<script>
import '../../theme/render.scss'
import { initEmptyArray } from '../../utils/helper'

export const getWrapParent = (dom) => {
  let el = dom
  if (!el) {
    return null
  }
...
export default {
  ...
  methods: {
    ...
    scroll(offset, up) {
       ...
       */
      const adjustUp = () => {
        const deltaRemain = remain < 5 ? 0 : remain / 2
        ...
        if (isSameHeight) {
          ...
          const index = Math.max((start - decreaseCount - deltaRemain) | 0, 0)
          this.start = index
          ...
          for (let i = start - 1; i >= 0; i--) {
            if (cache[i].top <= scrollTop) {
              const index = Math.max((i - deltaRemain) | 0, 0)
              this.paddingTop = cache[index].top
              this.start = index
              break
            }
    ...
    _computeRenderHeight(items, offset) {
      ...
      if (this.isSameHeight) {
        ...
        if (offset) {
          ...
          offsets = initEmptyArray(column, 0)
        }
        items.forEach((item, index) => {
    ...
    _filter(h) {
      ...
      const _start = start - (start % this.column)
      ...
      return this.$slots.default.slice(_start, end)
    }
  },
  render(h) {
    return h(
